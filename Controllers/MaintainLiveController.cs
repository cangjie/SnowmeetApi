    ...
    public class MaintainLiveController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<MaintainLive>>> GetTasksQuick(string key, string shop = "", string sessionKey = "")
        {
            ...
            var liveArr = await _context.MaintainLives.Include(m => m.taskLog).Include(m => m.order)
                ...
                (m.order_id.ToString().StartsWith(key) || m.order_id.ToString().EndsWith(key) || m.confirmed_cell.EndsWith(key)) ))
                .OrderByDescending(m => m.id).AsNoTracking().ToListAsync();
                /*
            ...
            string shop = "", string openId = "", string payOption = "")
        {
            ...
            var liveArr = await _context.MaintainLives.Include(m => m.taskLog)
                ...
                && (payOption.Trim().Equals("") || (!payOption.Trim().Equals("") && m.pay_memo.Trim().Equals(payOption.Trim())))))
                .AsNoTracking().OrderByDescending(m => m.id).ToListAsync();
